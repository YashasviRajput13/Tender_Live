    ...
    <div className="h-10 relative overflow-hidden flex justify-center items-center">
      <AnimatePresence mode="wait">
        <motion.span
          ...
          className="absolute text-[#C9A84C] font-display font-extrabold text-lg md:text-xl uppercase tracking-widest"
        >
          {texts[index]}
        </motion.span>
      </AnimatePresence>
  ...
  if (!token) {
    return (
      <div className="min-h-screen bg-white text-slate-900 overflow-x-hidden font-sans relative">
        {/* FLOATING HEADER */}
        <header className="fixed top-0 left-0 right-0 h-20 bg-[#050505]/70 backdrop-blur-md border-b border-white/[0.06] z-50 px-6 lg:px-12 flex items-center justify-between text-white">
          ...
          <nav className="hidden md:flex items-center space-x-10 text-sm font-medium text-slate-400">
            ...
            <a href="#demo" className="hover:text-white transition-colors duration-200">Preview</a>
          </nav>

          <div>
            <button 
              onClick={() => setShowLoginModal(true)}
              className="py-2.5 px-6 rounded-full bg-[#C9A84C] hover:bg-[#B8973B] text-[#050505] text-sm font-semibold transition-all hover:scale-[1.02] shadow-[0_2px_15px_rgba(201,168,76,0.25)]"
            >
              Sign In
            </button>
          </div>
        ...
        {/* HERO SECTION */}
        <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#050505] px-6 z-10 border-b border-white/5">
          ...
          {/* Film Grain Overlay */}
          {/* Noise bg removed as requested */}

          {/* Spotlight & Gradient Overlays */}
          <div className="absolute inset-0 z-10 pointer-events-none spotlight-radial-1" />
          <div className="absolute inset-0 z-10 pointer-events-none spotlight-radial-2" />

          {/* Hero Content Container */}
          <motion.div 
            style={{ y: contentY, opacity: contentOpacity }}
            className="max-w-5xl w-full mx-auto relative z-30 pt-32 pb-24 flex flex-col items-center text-center space-y-10"
          >
            <motion.div
              initial={{ opacity: 0, y: -15 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
              className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-white/[0.03] border border-white/10 text-xs text-[#E6C89C] font-mono uppercase tracking-[0.2em] shadow-[0_2px_10px_rgba(0,0,0,0.5)]"
            >
              <span className="w-1.5 h-1.5 bg-[#E6C89C] rounded-full animate-pulse"></span>
              <span>Why TenderAI</span>
            </motion.div>

            <motion.h1 
              initial="hidden"
              animate="visible"
              className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-[80px] font-display font-black text-white max-w-5xl select-none flex flex-col items-center tracking-tight leading-[1.15] text-center gap-y-3"
            >
              <motion.span 
                variants={{
              ...
              <motion.span 
                variants={{
                  hidden: { opacity: 0, y: 30, filter: "blur(8px)" },
                  visible: { opacity: 1, y: 0, filter: "blur(0px)", transition: { duration: 1, delay: 0.15, ease: [0.16, 1, 0.3, 1] } }
                }}
                className="block font-serif italic font-light text-[#E6C89C] my-1 py-1"
              >
            ...
            <motion.p 
              ...
              className="text-base md:text-[19px] text-[#A1A1AA] max-w-2xl mx-auto font-sans font-normal leading-relaxed tracking-normal"
            >
              Discover opportunities, analyze documents, match eligibility, and generate insights using autonomous AI agents.
            </motion.p>
            ...
            >
              <button 
                onClick={() => setShowLoginModal(true)}
                className="py-3.5 px-8 rounded-full bg-[#C9A84C] hover:bg-[#B8973B] text-[#050505] font-semibold text-sm transition-all hover:scale-[1.02] flex items-center space-x-2.5 shadow-[0_4px_25px_rgba(201,168,76,0.25)]"
              >
              ...
              <a 
                href="#demo"
                className="py-3.5 px-8 rounded-full bg-[#C9A84C]/5 hover:bg-[#C9A84C]/10 border border-[#C9A84C]/35 hover:border-[#C9A84C] text-sm font-semibold text-[#E6C89C] hover:text-white transition-all flex items-center space-x-2 hover:scale-[1.02] shadow-[0_4px_15px_rgba(201,168,76,0.05)]"
              >
                <Play className="w-3.5 h-3.5 text-[#E6C89C] fill-[#E6C89C]" />
            ...
            <div className="fixed inset-0 flex items-center justify-center z-50 p-6 select-text">
              ...
              >
                ...
                <div className="text-center space-y-3 mb-6">
                  <Logo variant="icon-only" className="h-16 w-16 mx-auto animate-pulse" />
                  <div>
                    <h1 className="text-2xl font-display font-extrabold text-slate-900 tracking-tight">TenderLive Login</h1>
                    <span className="text-sm text-[#C9A84C] font-semibold tracking-wide">Intelligence Terminal Access</span>
                  </div>
                </div>
                ...
                <form onSubmit={handleLoginSubmit} className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:border-[#C9A84C] focus:ring-2 focus:ring-[#C9A84C]/20 focus:outline-none text-base text-slate-900 placeholder:text-slate-400"
                  ...
                  <button
                    ...
                    className="w-full py-3.5 bg-[#C9A84C] hover:bg-[#B8973B] text-[#050505] font-bold text-base rounded-xl shadow-[0_0_25px_rgba(201,168,76,0.25)] hover:shadow-[0_0_35px_rgba(201,168,76,0.4)] hover:scale-[1.01] transition-all mt-2"
                  >
                    Sign In →
                  </button>
                </form>
