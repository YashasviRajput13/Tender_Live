...
class NotificationManager:
    @staticmethod
    def compile_evidence(event_type: str, eligibility_result: dict, score_result: int, tender_data: dict) -> tuple[str, list[str]]:
        """
        ...
        if event_type == "HIGH_MATCH":
            ...
            evidence.append(f"Tender Suitability Score of {score_result}/100 meets high-match standard.")
            evidence.append("Eligibility verdict evaluated as ELIGIBLE.")
            if financial_match.get("status") == "pass":
                evidence.append(f"Financial: {financial_match.get('details', 'Turnover requirements satisfied.')}")
            if technical_match.get("status") == "pass":
                evidence.append(f"Technical: {technical_match.get('details', 'Technical capabilities matched.')}")
            if experience_match.get("status") == "pass":
                evidence.append(f"Experience: {experience_match.get('details', 'Past projects demonstrate required capability.')}")
            if location_match.get("status") == "pass":
                evidence.append(f"Location: {location_match.get('details', 'Operational coverage aligns with tender location.')}")
            ...
            evidence.append(f"Tender Suitability Score of {score_result}/100 indicates medium suitability.")
            evidence.append(f"Eligibility Status: {eligibility_result.get('eligibility', 'partially_eligible').upper()}")
            if financial_match.get("status"):
                evidence.append(f"Financial status: {financial_match.get('status').upper()} ({financial_match.get('details', '')})")
            if technical_match.get("status"):
                evidence.append(f"Technical status: {technical_match.get('status').upper()} ({technical_match.get('details', '')})")
            if experience_match.get("status"):
                evidence.append(f"Experience status: {experience_match.get('status').upper()} ({experience_match.get('details', '')})")
            ...
            if eligibility_result.get("eligibility") == "not_eligible":
                evidence.append("Compliance Gap: Overall evaluation set to NOT ELIGIBLE.")
            
            if financial_match.get("status") == "fail":
                evidence.append(f"Financial Gap: {financial_match.get('details', 'Turnover criteria failed.')}")
            if technical_match.get("status") == "fail":
                evidence.append(f"Technical Gap: {technical_match.get('details', 'Capability mismatch.')}")
            if experience_match.get("status") == "fail":
                evidence.append(f"Experience Gap: {experience_match.get('details', 'Insufficient past project values.')}")
            if location_match.get("status") == "fail":
                evidence.append(f"Location Gap: {location_match.get('details', 'Tender is outside coverage area.')}")
            ...
            try:
                ...
                days_rem = (deadline_dt.replace(tzinfo=None) - datetime.utcnow().replace(tzinfo=None)).days
                evidence.append(f"Only {days_rem} days remaining until submission window closes.")
            except Exception:
    ...
    def calculate_priority(event_type: str, eligibility_result: dict) -> str:
        ...
        if technical_match.get("status") == "fail" and any(x in tech_details for x in ["certif", "iso", "gst", "licens", "missing"]):
            has_missing_cert = True

        if event_type == "RISK_ALERT":
            if (eligibility_result.get("eligibility") == "not_eligible" or
                financial_match.get("status") == "fail" or
                has_missing_cert):
                priority = "CRITICAL"
                logger.info("Escalating RISK_ALERT notification to CRITICAL due to major compliance/financial gap.")
    ...
    def send_notification_manager(
        ...
        company = db.query(models.Company).filter(
            func.lower(models.Company.name) == func.lower(company_data.get("name", ""))
        ).first()        
        tender = db.query(models.Tender).filter(models.Tender.tender_id == tender_data.get("tender_id")).first()

        if not company or not tender:
            logger.error(f"Cannot process notification. Company found: {bool(company)}, Tender found: {bool(tender)}")
            return False
        ...
        if raw_title.startswith("Analyzed:"):
            raw_title = raw_title.split("_", 2)[-1].replace("_", " ").replace(".pdf", "").strip()
        ...
        if existing_notif:
            logger.info(f"Skipping duplicate notification: Type: {event_type}, Tender: {tender_id}, Company: {company_id}")
            return True

        # 2. Compile evidence list and trigger reason deterministically
        trigger_reason, evidence_list = cls.compile_evidence(event_type, eligibility_result, score_result, tender_data)
        ...
        try:
            ...
            logger.warning(f"LLM summaries generation failed: {str(e)}. Using fallback deterministic texts.")
        ...
        if event_type == "HIGH_MATCH":
            ...
            title = "🚨 CRITICAL Tender Risk Alert" if priority == "CRITICAL" else "🚨 Tender Risk Alert"
        ...
        logger.info(f"[notification_created] Generated alert ID: {new_notif.id} for Tender ID: {tender_id}, Company ID: {company_id}, Type: {event_type}, Priority: {priority}")
        ...
        recent_emails = db.query(models.AuditLog).filter(
        ...
        for log in recent_emails:
            ...
            if (str(details.get("tender_id")) == str(tender_id) and 
                str(details.get("type")) == event_type):
                email_throttled = True
                break

        if email_throttled:
            logger.info(f"Email notifications throttled: Type {event_type} already sent within 24 hours for Tender ID: {tender_id}")

        # 8. Send Email if not throttled
        email_sent_successfully = False
        if not email_throttled:
            # Fetch recipient user email
            user = db.query(models.User).filter(models.User.id == company.user_id).first()
            if user and user.email:
                html_body = ""
                if event_type == "HIGH_MATCH":
                    html_body = render_high_match_email(
                        company_name=str(company.name or ""),
                        tender={k: (str(v) if v is not None else "Not specified") for k, v in tender_data.items()},
                        score=score_result,
                        eligibility_verdict=str(eligibility_result.get("eligibility") or "eligible"),
                    ...
                    html_body = render_medium_match_email(
                        ...
                        tender={k: (str(v) if v is not None else "Not specified") for k, v in tender_data.items()},
                        score=score_result,
                        eligibility_verdict=str(eligibility_result.get("eligibility") or "partially_eligible"),
                        why_matches=str(why_matches or ""),
                        recommended_action=str(recommended_action or ""),
                        evidence=evidence_list or []
                    )
                    ...
                    checklist = eligibility_result.get("checklist", {}).get("submission_checklist", [])
                    if not checklist:
                        checklist = ["Valid registration proof", "Financial balance sheets", "MSME Certificate"]
                ...
                if email_sent_successfully:
                    ...
                    email_audit = models.AuditLog(
                        user_id=company.user_id,
                        action="email_sent",
                        details={"tender_id": tender_id, "company_id": company_id, "type": event_type}
                    )
                    db.add(email_audit)
                    db.commit()
        ...
        run_async(sse_manager.publish("dashboard_events", "notification_added", sse_payload))
