...
class EligibilityAgent:
    def analyze(
        self, company_profile: Dict[str, Any], tender_details: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Running Eligibility analysis for company {company_profile.get('name')} and tender {tender_details.get('tender_id')}"
        )
        ...
        try:
            ...
            eligibility = (
                "eligible"
                if len(overlap) >= 2
                else ("partially_eligible" if overlap else "not_eligible")
            )
            return {
                ...
                "financial_match": {
                    "status": "conditional",
                    "details": "AI unavailable — financial check skipped",
                },
                "technical_match": {
                    "status": "pass" if overlap else "fail",
                    "details": f"Sector overlap: {overlap}",
                },
                "experience_match": {
                    "status": "conditional",
                ...
                "msme_advantage": {
                    "applicable": bool(company_profile.get("msme_status")),
                    "details": "MSME status from profile",
                },
                "location_match": {
                    "status": "pass",
                    "details": "AI unavailable — defaulting to pass",
                },
