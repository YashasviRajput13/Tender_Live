...
from reportlab.graphics.shapes import Drawing, String, Rect, Group, Wedge
...
class ProgressBar(Flowable):
    def __init__(self, width, height, percentage, label="", color=colors.HexColor('#C9A84C')):
    ...
    def draw(self):
        ...
        c.setFont("Helvetica-Bold", 10)
        c.drawString(bar_x + bar_w + 10, self.height/2 - 3, f"{int(self.percentage)}%")
...
class CircularGauge(Flowable):
    ...
    def draw(self):
        ...
        c.wedge(center_x, center_y, radius, start_angle, start_angle + extent, fill=1, stroke=0)
...
def generate_evaluation_report(tender, company, report, file_path):
    ...
    title_style = ParagraphStyle('CoverTitle', parent=styles['Heading1'], fontSize=24, textColor=colors.HexColor('#0F172A'), spaceAfter=20, alignment=1)
    h1_style = ParagraphStyle('H1', parent=styles['Heading1'], fontSize=20, textColor=colors.HexColor('#C9A84C'), spaceAfter=15, spaceBefore=20)
    h2_style = ParagraphStyle('H2', parent=styles['Heading2'], fontSize=16, textColor=colors.HexColor('#1E293B'), spaceBefore=15, spaceAfter=10)
    body_style = ParagraphStyle('Body', parent=styles['Normal'], fontSize=11, textColor=colors.HexColor('#334155'), spaceAfter=8, leading=16)
    ...
    info_data = [
        ...
        [Paragraph("<b>Generated On:</b>", body_style), datetime.now().strftime('%d %B %Y')],
    ]
    ...
    story.append(Paragraph(f"<b>Recommendation:</b> <font color='{color}'>● {recommendation}</font>", h2_style))
    ...
    story.append(Paragraph(f"<b>Eligibility Match:</b> {int(eligibility_score)}%", h2_style))
    ...
    for label, val in skills:
        story.append(ProgressBar(300, 15, val, label=label, color=colors.HexColor('#3B82F6')))
        story.append(Spacer(1, 10))
    ...
    for r, status, c in reqs:
        p = Paragraph(f"<b>{r}</b>: <font color='{c.hexval()}'>{status}</font>", body_style)
        story.append(p)
        story.append(Spacer(1, 10))
    ...
    story.append(Paragraph("<font color='red'>❌ CMMI Level 3 Certification</font>", bold_body))
    story.append(Paragraph("Impact: -5%", body_style))
    story.append(Spacer(1, 10))
    
    story.append(Paragraph("<font color='red'>❌ One Additional Similar Project</font>", bold_body))
    story.append(Paragraph("Impact: -3%", body_style))
    story.append(Spacer(1, 20))
    
    story.append(Paragraph(f"<b>Total Score Reduction:</b> -8%", h2_style))
    story.append(Paragraph(f"<b>Current Score:</b> {int(score)}%", body_style))
    story.append(Paragraph(f"<b>Potential Score:</b> {min(100, score + 8)}% (if missing requirements fulfilled)", body_style))
    story.append(PageBreak())
    ...
    calc_data = [
        ...
        ["Eligibility", "30%", f"{int(eligibility_score)}", f"{eligibility_score * 0.30:.1f}"],
    ...
    story.append(Paragraph(f"<b>Predicted Win Probability:</b> {int(win_prob)}%", h2_style))
    story.append(Spacer(1, 20))
    ...
    def add_footer(canvas, doc):
        ...
        canvas.drawString(40, 20, f"TenderLive Evaluation Report - Generated {datetime.now().strftime('%Y-%m-%d %H:%M')}")
