...
class ScoringAgent:
    ...
    SECTOR_KEYWORDS = {
        "it_software": [
            "software", "application", "portal", "web", "mobile", "app",
            "digital", "erp", "crm", "saas", "platform", "system integration",
            "e-governance", "egovernance", "it service", "ict"
        ],
        "cloud_infra": [
            "cloud", "hosting", "infrastructure", "server", "datacenter",
            "data center", "colocation", "iaas", "paas", "migration", "devops"
        ],
        "cybersecurity": [
            "security", "cybersecurity", "firewall", "soc", "vapt",
            "penetration testing", "siem", "endpoint", "iso 27001", "compliance"
        ],
        "ai_ml": [
            "artificial intelligence", "machine learning", "ai", "ml",
            "analytics", "data science", "nlp", "computer vision", "chatbot"
        ],
        "networking": [
            "network", "wan", "lan", "vpn", "connectivity", "bandwidth",
            "fiber", "leased line", "mpls", "switching", "routing"
        ],
        "civil_construction": [
            "road", "civil", "construction", "bridge", "building", "concrete",
            "rcc", "structural", "foundation", "earthwork", "drainage"
        ],
        "electrical": [
            "electrical", "power", "transformer", "substation", "wiring",
            "generator", "ups", "solar", "renewable energy"
        ],
        "mechanical": [
            "mechanical", "hvac", "plumbing", "pump", "valve", "piping",
            "fabrication", "welding", "equipment supply"
        ],
        "printing_media": [
            "printing", "publication", "media", "broadcast", "content",
            "advertising", "design", "branding"
        ],
        "medical_health": [
            "medical", "hospital", "health", "pharmaceutical", "equipment supply",
            "diagnostic", "ambulance", "nursing"
        ],
    }
    ...
    def _score_industry_match(self, company: Dict[str, Any], tender: Dict[str, Any]) -> tuple[int, str]:
        """
        ...
        if not overlap:
            return 0, f"No sector overlap. Company: {company_sectors}. Tender: {tender_sectors}."

        match_ratio = len(overlap) / max(len(tender_sectors), 1)
        points = round(min(30, match_ratio * 30 + (10 if len(overlap) >= 2 else 0)))
        return points, f"Matched sectors: {overlap}. Company sectors: {company_sectors}."

    def _score_technical_capability(self, company: Dict[str, Any], tender: Dict[str, Any], eligibility: Dict[str, Any]) -> tuple[int, str]:
        """
        ...
        tender_text = self._normalize_text(f"{tender.get('title', '')} {tender.get('eligibility_criteria', '')}")

        cert_bonus = 0
        cert_keywords = ["iso 9001", "iso 27001", "cmmi", "ce mark", "bis", "gst registered"]
        for cert in cert_keywords:
            if cert in tender_text and cert in company_certs:
                cert_bonus += 2

        points = min(25, base + cert_bonus)
        return points, f"Technical status: {status}. Cert bonus: {cert_bonus}pts. Details: {tech_match.get('details', '')}."

    def _score_past_experience(self, company: Dict[str, Any], tender: Dict[str, Any], eligibility: Dict[str, Any]) -> tuple[int, str]:
        """
        ...
        return points, f"Experience status: {status}. Budget/Turnover ratio: {round(tender_budget / max(company_turnover_rupees, 1), 2)}. Value bonus: {value_bonus}pts."

    def _score_certifications(self, company: Dict[str, Any], tender: Dict[str, Any]) -> tuple[int, str]:
        """
        ...
        cert_map = {
            "iso 9001": 3, "iso 27001": 3, "cmmi": 3,
            "msme": 2, "nsic": 2, "gem": 1, "gst": 1
        }
    ...
    def _score_financial_eligibility(self, company: Dict[str, Any], tender: Dict[str, Any], eligibility: Dict[str, Any]) -> tuple[int, str]:
        """
    ...
    def _score_geographic_match(self, company: Dict[str, Any], tender: Dict[str, Any], eligibility: Dict[str, Any]) -> tuple[int, str]:
        """
        ...
        if any(loc in ["india", "pan india", "pan-india", "all india", "nationwide"] for loc in company_locations):
            return 5, "Company has pan-India coverage."

        if any(loc in tender_location or tender_location in loc for loc in company_locations):
            return 5, f"Location '{tender_location}' covered by company."

        return 0, f"Company coverage {company_locations} does not include '{tender_location}'."
    ...
    def calculate(
        ...
        """
        logger.info(f"[ScoringAgent] Starting deterministic scoring for tender {tender_details.get('tender_id')}")

        industry_pts, industry_reason   = self._score_industry_match(company_profile, tender_details)
        technical_pts, technical_reason = self._score_technical_capability(company_profile, tender_details, eligibility_report)
        experience_pts, exp_reason      = self._score_past_experience(company_profile, tender_details, eligibility_report)
        cert_pts, cert_reason           = self._score_certifications(company_profile, tender_details)
        financial_pts, fin_reason       = self._score_financial_eligibility(company_profile, tender_details, eligibility_report)
        geo_pts, geo_reason             = self._score_geographic_match(company_profile, tender_details, eligibility_report)

        total = industry_pts + technical_pts + experience_pts + cert_pts + financial_pts + geo_pts
