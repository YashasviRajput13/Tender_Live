...
def generate_reports(
    ...
    log_messages = [
        {
            ...
            "level": "INFO",
            "message": f"Report Generation started. Format: {format_type.upper()}",
        }
    ]
    ...
    _fire_task_update(
        task_id, 10, "running", "completed", "Generating reports...", log_messages
    )
    ...
    try:
        ...
        if format_type.lower() == "pdf":
            file_name = (
                f"TenderAI_Analysis_Brief_{tender_id}_{timestamp_str}.pdf"
                if tender_id
                else f"TenderAI_Analysis_Brief_{timestamp_str}.pdf"
            )
            ...
            if tender_id:
                reports = (
                    db.query(models.EligibilityReport)
                    .filter(
                        models.EligibilityReport.company_id == company.id,
                        models.EligibilityReport.tender_id == tender_id,
                    )
                    .all()
                )
            else:
                reports = (
                    db.query(models.EligibilityReport)
                    .filter(models.EligibilityReport.company_id == company.id)
                    .all()
                )
            ...
            log_messages.append(
                {
                    "timestamp": str(datetime.utcnow()),
                    "level": "INFO",
                    "message": f"Assembling {len(reports)} tender matches into PDF briefing templates...",
                }
            )
            task.log_messages = log_messages
            db.commit()
            ...
            # Cover Page
            story.append(
                Paragraph("TenderAI Opportunity Assessment Brief", title_style)
            )
            story.append(Paragraph(f"Prepared for: <b>{company.name}</b>", body_style))
            story.append(
                Paragraph(f"Industry: <b>{company.industry or 'N/A'}</b>", body_style)
            )
            story.append(
                Paragraph(
                    f"Annual Turnover: <b>{float(company.turnover or 0):,.2f} Lakhs</b>",
                    body_style,
                )
            )
            story.append(
                Paragraph(
                    f"MSME Status: <b>{'Registered' if company.msme_status else 'Not Registered'}</b>",
                    body_style,
                )
            )
            story.append(
            ...
            story.append(
                Paragraph(
                    f"Active Opportunity matches evaluated: <b>{len(reports)}</b>",
                    body_style,
                )
            )
            ...
            for r in reports:
                tender_obj = (
                    ...
                    .filter(models.Tender.id == r.tender_id)
                    .first()
                )
                if tender_obj:
                    table_data.append(
                        [
                            ...
                            tender_obj.title[:40] + "..."
                            if len(tender_obj.title) > 40
                            else tender_obj.title,
                            r.eligibility.upper(),
            ...
            for r in reports:
                ...
                # Title
                story.append(
                    Paragraph(f"Tender Opportunity: {tender_obj.title}", h2_style)
                )
                ...
                financial_info = (
                    r.timeline.get("financial_info", {})
                    if getattr(r, "timeline", None)
                    else {}
                )
                ...
                for doc in tender_obj.documents:
                    if doc.parsed_json and "financial_requirements" in doc.parsed_json:
                        ...
                        if (
                            emd_val == "Not specified"
                            and f_reqs.get("earnest_money_deposit", "Not specified")
                            != "Not specified"
                        ):
                            emd_val = f_reqs.get("earnest_money_deposit")
                        if (
                            perf_val == "Not specified"
                            and f_reqs.get("performance_security", "Not specified")
                            != "Not specified"
                        ):
                ...
                info_data = [
                    ...
                    [
                        "Tender Type:",
                        "Open Tender"
                        if not tender_obj.source_name
                        else tender_obj.source_name,
                    ],
                    ["Tender Category:", "Not specified"],
                    [
                        "Tender Value:",
                        f"₹ {tender_obj.budget:,.2f}"
                        if tender_obj.budget
                        else "Not specified",
                    ],
                    ...
                    [
                        ...
                        tender_obj.created_at.strftime("%d %b %Y"),
                    ],
                    [
                        "Bid Submission End Date:",
                        tender_obj.deadline.strftime("%d %b %Y")
                        if tender_obj.deadline
                        else "Not specified",
                    ],
                ]
                ...
                story.append(
                    Paragraph(
                        f"<b>Overall Match Score: {r.opportunity_score}%</b>",
                        body_style,
                    )
                )
                ...
                for key, label in [
                    ("financial_match", "Financial Eligibility"),
                    ("technical_match", "Technical Qualifications / Product Match"),
                    ("experience_match", "Experience Match"),
                    ("location_match", "Location Suitability"),
                ]:
                    match_info = req_analysis.get(key, {})
                    if match_info:
                        score_data.append(
                            [label, match_info.get("status", "conditional").upper()]
                        )
                ...
                story.append(
                    Paragraph(r.summary or "No summary available.", body_style)
                )
                story.append(Spacer(1, 15))

                # 4. Required Products/Services
                story.append(
                    Paragraph("<b>4. Required Products/Services</b>", h2_style)
                )
                ...
                if key_reqs:
                    ...
                    story.append(
                        Paragraph(
                            "<i>No specific products/services extracted.</i>",
                            body_style,
                        )
                    )
                story.append(Spacer(1, 15))
                ...
                if reasons:
                    ...
                    story.append(
                        Paragraph(
                            "No direct matching criteria explicitly confirmed.",
                            body_style,
                        )
                    )
                story.append(Spacer(1, 15))
                ...
                for key, label in [
                    ...
                    if match_info:
                        eligibility_criteria.append(
                            f"<b>{label}:</b> {match_info.get('details', '')}"
                        )
                ...
                if risk_items:
                    story.append(
                        Paragraph(
                            f"Risk Level: <b>{'High' if len(risk_items) >= 3 else 'Medium'}</b>",
                            body_style,
                        )
                    )
                    story.append(Spacer(1, 6))
                    ...
                    story.append(
                        Paragraph(
                            "Risk Level: <b>Low</b> (No significant risks identified by AI)",
                            body_style,
                        )
                    )
                story.append(Spacer(1, 15))
                ...
                fin_data = [
                    ...
                    [
                        "Bid Validity Period",
                        financial_info.get("bid_validity", "Not specified"),
                    ],
                ]
                ...
                dl_data = [
                    ...
                    [
                        "Last Submission:",
                        tender_obj.deadline.strftime("%d %b %Y %H:%M")
                        if tender_obj.deadline
                        else "Not specified",
                    ],
                ...
                story.append(
                    Paragraph(
                        f"<b>Delivery/Work Location:</b> {tender_obj.location or 'Not specified'}",
                        body_style,
                    )
                )
                story.append(Spacer(1, 15))
                ...
                story.append(
                    Paragraph(
                        f"<b>Authority Source:</b> {tender_obj.source_name}", body_style
                    )
                )
                if tender_obj.source_url:
                    story.append(
                        Paragraph(
                            f"<b>Source URL:</b> <a href='{tender_obj.source_url}' color='blue'>{tender_obj.source_url}</a>",
                            body_style,
                        )
                    )
                else:
                    story.append(
                        Paragraph("<i>Source URL not specified.</i>", body_style)
                    )
                story.append(Spacer(1, 15))

                # 13. Competitor & Win Probability
                story.append(
                    Paragraph("<b>13. Competitor & Win Probability</b>", h2_style)
                )
                story.append(
                    Paragraph(
                        f"Win Probability: <b>{r.confidence_score * 100:.1f}%</b> (Estimated based on initial compliance mapping)",
                        body_style,
                    )
                )
                story.append(
                    Paragraph(
                        "<i>Note: Advanced competitor pricing models are currently building historical datasets.</i>",
                        body_style,
                    )
                )
                story.append(Spacer(1, 15))
                ...
                recommendation = (
                    ...
                    if r.eligibility.lower() == "eligible"
                        ...
                        if r.eligibility.lower() == "partially_eligible"
                ...
                story.append(
                    Paragraph(
                        f"AI Recommendation: <font color='{color}'><b>{recommendation}</b></font>",
                        body_style,
                    )
                )
                story.append(
                    Paragraph(
                        f"Confidence Score: {r.confidence_score * 100:.1f}%", body_style
                    )
                )
                ...
                if r.eligibility.lower() == "eligible":
                    ...
                    if not risk_items:
                        recs_reason.append("Low compliance risk.")
                else:
                    recs_reason.append(
                        "Does not meet all primary eligibility thresholds."
                    )
            ...
            file_name = (
                f"TenderAI_Catalog_{tender_id}_{timestamp_str}.xlsx"
                if tender_id
                else f"TenderAI_Catalog_{timestamp_str}.xlsx"
            )
            file_path = os.path.join(settings.UPLOAD_DIR, file_name)
            ...
            log_messages.append(
                {
                    "timestamp": str(datetime.utcnow()),
                    "level": "INFO",
                    "message": f"Mapping {len(reports)} matching items into Excel catalogs...",
                }
            )
            task.log_messages = log_messages
            db.commit()
                    ...
                    records.append(
                        {
                            ...
                            "Budget (INR)": float(tender_obj.budget)
                            if tender_obj.budget
                            else None,
                            "Deadline": tender_obj.deadline.strftime("%Y-%m-%d %H:%M")
                            if tender_obj.deadline
                            else "Not specified",
                            ...
                            "Discovery Date": tender_obj.created_at.strftime(
                                "%Y-%m-%d"
                            ),
                        }
                    )
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": f"Report generated successfully: {file_name}",
            }
        )
        task.progress = 100
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "ERROR",
                "message": f"Error rendering report sheet: {str(e)}",
            }
        )
        task.status = "failed"
        task.progress = 100
