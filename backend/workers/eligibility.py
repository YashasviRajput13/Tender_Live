...
def run_eligibility_matching(
    task_id: str, tender_db_id: int, company_db_id: int = None
):
    """
    ...
    if company_db_id:
        company = (
            db.query(models.Company).filter(models.Company.id == company_db_id).first()
        )
        if not company:
            ...
            return False
        logger.info(
            f"Using company profile: '{company.name}' (ID: {company.id}) for eligibility matching."
        )
    else:
        ...
        if not company:
            ...
            log_messages = [
                {
                    ...
                    "level": "ERROR",
                    "message": "No active Company Profile found. Please configure company details first.",
                }
            ]
            task.status = "failed"
            task.progress = 100
            ...
            return False
        logger.warning(
            f"No company_db_id provided — falling back to first company: '{company.name}' (ID: {company.id})."
        )
    ...
    log_messages = [
        {
            "timestamp": str(datetime.utcnow()),
            "level": "INFO",
            "message": f"AI Multi-Agent Pipeline started. Evaluator target: {company.name}",
        }
    ]
    task.log_messages = log_messages
    db.commit()
    ...
    run_async(
        trigger_task_update(
            ...
            "running",
            "eligibility_agent",
            "Starting AI eligibility analysis...",
            log_messages,
        )
    )
    ...
    try:
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": "Invoking Eligibility Agent. Evaluating criteria...",
            }
        )
        task.log_messages = log_messages
        db.commit()
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": f"Eligibility matched: {eligibility_res.get('eligibility')} (Confidence: {eligibility_res.get('confidence_score')})",
            }
        )
        ...
        run_async(
            trigger_task_update(
                ...
                "running",
                "summary_agent",
                f"Eligibility Agent completed: {eligibility_res.get('eligibility')}",
                log_messages,
            )
        )
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": "Invoking Summarization Agent. Distilling risks and milestones...",
            }
        )
        task.log_messages = log_messages
        db.commit()
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": "Summarization finished. Document timeline mapped.",
            }
        )
        ...
        run_async(
            trigger_task_update(
                ...
                "running",
                "scoring_agent",
                "Summarization complete. Scoring tender...",
                log_messages,
            )
        )
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": "Invoking Opportunity Scoring Agent. Calculating suitability scale...",
            }
        )
        task.log_messages = log_messages
        db.commit()
        run_async(
            trigger_task_update(
                ...
                "running",
                "scoring_agent",
                "Opportunity Scoring Agent executing...",
                log_messages,
            )
        )
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": f"Opportunity Score calculated: {score_res}/100.",
            }
        )
        task.log_messages = log_messages
        task.progress = 90
        ...
        run_async(
            trigger_task_update(
                ...
                "running",
                "scoring_agent",
                f"Scoring finished. Score: {score_res}/100",
                log_messages,
            )
        )
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": "Multi-agent report compiled and persisted in PostgreSQL.",
            }
        )
        ...
        if tender.deadline:
            days_remaining = (
                tender.deadline.replace(tzinfo=None)
        ...
        for event_type in triggered_events:
            logger.info(
                f"Queueing notification: {event_type} for Tender ID: {tender.id}"
            )
            try:
                ...
                logger.error(
                    f"FAILED to queue notification task for {event_type}: {str(e)}"
                )
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "ERROR",
                "message": f"Error running agent pipeline: {str(e)}",
            }
        )
        task.status = "failed"
        task.progress = 100
