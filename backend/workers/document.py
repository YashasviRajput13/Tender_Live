...
def process_tender_document(task_id: str, tender_db_id: int, file_path: str):
    ...
    _fire_task_update(task_id, 10, "running", "document_intel", "Document processing started", log_messages)
    ...
    try:
        ...
        task.log_messages = log_messages
        db.commit()
        _fire_task_update(task_id, 30, "running", "document_intel", "Extracting PDF text layers...", log_messages)
        ...
        task.log_messages = log_messages
        db.commit()
        _fire_task_update(task_id, 70, "running", "document_intel", "Structuring criteria JSON...", log_messages)
        ...
        _fire_task_update(task_id, 100, "completed", "completed", "Document analyzed. Triggering eligibility pipeline...", log_messages)
        ...
        try:
            ...
            logger.warning(f"Redis unavailable for eligibility dispatch, falling back to thread: {str(celery_err)}")
            threading.Thread(
        ...
        _fire_task_update(task_id, 100, "failed", "completed", f"Document processing failed: {str(e)}", log_messages)
        db.close()
        return False
