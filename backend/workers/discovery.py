...
def run_tender_discovery(task_id: str):
    ...
    log_messages = [
        {
            ...
            "level": "INFO",
            "message": "Scraping Engine initialized. Targeting active government procurement pages.",
        }
    ]
    ...
    run_async(
        trigger_task_update(
            task_id, 10, "running", "scraper", "Scraping started", log_messages
        )
    )
    ...
    try:
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": "Connecting to Government eMarketplace (GeM) Bid Portal...",
            }
        )
        task.log_messages = log_messages
        db.commit()
        run_async(
            trigger_task_update(
                task_id, 20, "running", "scraper", "Scanning GeM bids...", log_messages
            )
        )
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": f"GeM scan finished. Found {len(gem_tenders)} active bids.",
            }
        )
        task.log_messages = log_messages
        db.commit()
        run_async(
            trigger_task_update(
                task_id, 40, "running", "scraper", "GeM Scan completed", log_messages
            )
        )
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": "Connecting to Central Public Procurement Portal (CPPP)...",
            }
        )
        task.log_messages = log_messages
        db.commit()
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": f"CPPP scan finished. Found {len(cppp_tenders)} active tenders.",
            }
        )
        task.log_messages = log_messages
        db.commit()
        run_async(
            trigger_task_update(
                task_id, 70, "running", "scraper", "CPPP Scan completed", log_messages
            )
        )
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "INFO",
                "message": f"Saving {len(all_tenders)} scraped tenders to database...",
            }
        )
        task.log_messages = log_messages
        db.commit()
        run_async(
            trigger_task_update(
                ...
                "running",
                "scraper",
                "Normalizing and saving tender records...",
                log_messages,
            )
        )
        ...
        for tender_data in all_tenders:
            existing = (
                ...
                .filter(models.Tender.tender_id == tender_data["tender_id"])
                .first()
            )
            if existing:
                duplicate_count += 1
            ...
            if tender_data.get("source_url"):
                logger.info(
                    f"Saving tender {tender_data['tender_id']} with source_url={tender_data['source_url']}"
                )
            else:
                warning_msg = (
                    f"Saving tender {tender_data['tender_id']} with missing source_url"
                )
            ...
            run_async(
                sse_manager.publish(
                    ...
                    "tender_discovered",
                    {
                        ...
                        "budget": float(new_tender.budget)
                        if new_tender.budget
                        else None,
                        "deadline": new_tender.deadline.isoformat()
                        if new_tender.deadline
                        else None,
        ...
        run_async(
            trigger_task_update(
                ...
                "completed",
                "scraper",
                f"Discovery finished. Saved {saved_count} new tenders. Auto-analysing...",
                log_messages,
            )
        )
        ...
        if new_tender_ids:
            ...
            if not company:
                ...
                logger.info(
                    f"Auto-queuing AI analysis for {len(new_tender_ids)} new tender(s) using profile: {company.name}"
                )
                ...
                for tender_db_id in new_tender_ids:
                    ...
                    db.commit()
                    logger.info(
                        f"Auto-queuing analysis task {analysis_task_id} for tender ID {tender_db_id}"
                    )
                    try:
                        run_eligibility_matching.delay(
                            analysis_task_id, tender_db_id, company.id
                        )
                    except Exception as e:
                        logger.warning(
                            f"Celery unavailable for auto-analysis, running in thread: {e}"
                        )
                        import threading
        ...
        log_messages.append(
            {
                "timestamp": str(datetime.utcnow()),
                "level": "ERROR",
                "message": f"Error running crawler: {str(e)}",
            }
        )
        task.status = "failed"
        task.progress = 100
