...
def start_task(
    ...
    new_task = models.AgentTask(
        ...
        status="pending",
        progress=0,
        current_agent="scraper" if req.task_type == "discovery" else "eligibility_agent"
    )
    db.add(new_task)
    db.commit()
    db.refresh(new_task)
    logger.info(f"Received task start request. task_type={req.task_type}, user={current_user.email}")
    ...
    if req.task_type == "discovery":
        try:
            ...
            logger.warning(f"Redis/Celery unavailable, falling back to local discovery execution: {str(e)}")
            threading.Thread(target=run_tender_discovery.run, args=(task_id,), daemon=True).start()
    ...
    # 2. Trigger Eligibility & Multi-agent Analysis
        ...
        try:
            ...
            logger.info(f"Triggered background Multi-agent Analysis on Tender {tender.id}. Task ID: {task_id}")
        except Exception as e:
            logger.warning(f"Redis/Celery unavailable, falling back to local analysis execution: {str(e)}")
            threading.Thread(target=run_eligibility_matching.run, args=(task_id, tender.id, company.id), daemon=True).start()
...
@router.get("",
            response_model=List[schemas.AgentTaskOut])
def list_tasks(
    db: Session = Depends(get_db),
    current_user: models.User = Depends(auth.get_current_user)
):
