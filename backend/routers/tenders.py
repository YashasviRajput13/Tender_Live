...
def list_tenders(
    ...
    status: Optional[str] = Query(
        None, description="Filter by status discovered, processing, analyzed"
    ),
    ...
    """
    query = db.query(models.Tender).options(
        joinedload(models.Tender.eligibility_reports)
    )
    ...
    def sort_key(t):
        best_score = max(
            (r.opportunity_score for r in t.eligibility_reports), default=-1
        )
...
def get_tender_detail(
    ...
    if not tender:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Tender record not found."
        )
    ...
    return {"tender": tender_schema, "eligibility_report": report_data}
...
def download_single_tender_summary(
    ...
    info_data = [
        ...
        [
            "Deadline:",
            tender.deadline.strftime("%d %b %Y %H:%M")
            if tender.deadline
            else "Not specified",
        ],
    ...
    if tender.source_url:
        story.append(
            Paragraph(
                f"<b>Source URL:</b> <a href='{tender.source_url}' color='blue'>{tender.source_url}</a>",
                body_style,
            )
        )
        story.append(Spacer(1, 15))
    ...
    if has_ai_report:
        ...
        story.append(
            Paragraph(
                "<i>AI Summary unavailable, tender data exported successfully.</i>",
                body_style,
            )
        )
    story.append(Spacer(1, 15))
    ...
    story.append(
        Paragraph(
            tender.eligibility_criteria or "None specified in basic data.", body_style
        )
    )
    story.append(Spacer(1, 15))
    ...
    if has_ai_report:
        risks = (
            report.risk_analysis.get("risks", [])
            if getattr(report, "risk_analysis", None)
            else []
        )
        if risks:
            for r in risks:
    ...
    if has_ai_report:
        recommendation = (
            ...
            if report.eligibility.lower() == "eligible"
            else (
                "MODERATELY RECOMMENDED"
                if report.eligibility.lower() == "partially_eligible"
        ...
        story.append(
            Paragraph(
                f"Suitability Score: <b>{report.opportunity_score}/100</b>", body_style
            )
        )
        story.append(
            Paragraph(
                f"Decision: <font color='{color}'><b>{recommendation}</b></font>",
                body_style,
            )
        )
    else:
