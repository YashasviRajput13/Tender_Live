...
def trigger_report_compilation(
    ...
    tender_id: int = Query(
        None, description="Optional tender ID for a specific tender report"
    ),
    ...
    if format_type not in ("pdf", "excel"):
        raise HTTPException(
            status_code=400, detail="Unsupported format type. Use 'pdf' or 'excel'."
        )
    ...
    try:
        ...
        logger.info(
            f"Triggered background {format_type.upper()} report generation. Task ID: {task_id}"
        )
    except Exception as celery_err:
        logger.warning(
            f"Redis/Celery unavailable for report task, falling back to local thread: {str(celery_err)}"
        )
        threading.Thread(
...
def download_report_file(
    file_name: str, current_user: models.User = Depends(auth.get_current_user)
):
    """
    ...
    if not (
        file_name.startswith("TenderAI_Analysis_Brief_")
        or file_name.startswith("TenderAI_Catalog_")
    ):
        raise HTTPException(
            status_code=403, detail="Forbidden. Access to system paths restricted."
        )
...
def download_evaluation_report(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to generate the evaluation report."
        )
